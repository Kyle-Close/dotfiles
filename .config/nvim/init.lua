...
vim.api.nvim_create_autocmd("BufWritePre", {
  pattern = { "*.ts", "*.js", "*.lua", "*.tsx", "*.json" }, -- Add file types here
  callback = function()
    vim.lsp.buf.format()                                   -- This triggers the formatting
  end,
})
...
-- Handle setting up lazy plugin manager. If it's not installed this will handle it
...
require("lazy").setup({
  spec = {
    ...
    { -- Fuzzy Finder (files, lsp, etc)
      ...
      config = function()
        ...
        require("telescope").setup({
          ...
          --
          defaults = {
            file_ignore_patterns = { "node_modules/", ".git/", "build/", "*.log" },
          },
          -- pickers = {}
          extensions = {
            ["ui-select"] = {
    ...
    },
    {
      ...
      config = function()
        -- On load we can access null-ls functions to configure what we need
        local null_ls = require("null-ls")

        -- Tell null-ls to use prettier as formatter
        null_ls.setup({
          sources = {
      ...
      config = function()
        ...
        for _, language in ipairs(js_based_languages) do
          dap.configurations[language] = {
            ...
            -- Debug node js processes (make sure to add --inspect)
            {
              type = "pwa-node",
              request = "attach",
              name = "Attach",
      ...
      dependencies = {
        {
          ...
          build = "npm install --legacy-peer-deps && npm gulp vsDebugServerBundle && mv dist out",
        },
        {
...
-- Load the theme after plugin work is done
